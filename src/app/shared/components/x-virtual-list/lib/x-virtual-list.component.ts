...
import {
  ...
  MOUSE_DOWN, MOUSE_UP, MOUSE_LEAVE, MOUSE_OUT, TOUCH_END, TOUCH_LEAVE, TOUCH_OUT, TOUCH_START, MARGIN_TOP, MARGIN_LEFT,
  SCROLLER_WHEEL, SCROLLER_SCROLLBAR_SCROLL,
} from './const';
...
export class XVirtualListComponent implements OnDestroy {
  ...
  constructor() {
    ...
    this.$prepared.pipe(
      ...
      tap(v => {
        readyToStart = v;
        this.classes.set({ prepared: true, 'ready-to-start': true });
      }),
    ).subscribe();
